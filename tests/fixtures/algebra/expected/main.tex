...
\texttt{[unsupported Typst element: state-update]}
...
\texttt{[unsupported Typst element: context]}
...
\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{\ensuremath{F}-algebra}
\end{figure}


This doesn"t cover all of the group properties yet. We are still missing associativity and the inverse laws. Unfortunately, the functor \ensuremath{F} primarily deals with the types of operations in an algebra, also known as the "signature", so the extra laws must be separately required. \texttt{[unsupported Typst element: footnote]} For example, we will define groups as follows:

\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]}
\caption{}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{\ensuremath{F}-algebra morphism}
\end{figure}
 

\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \textbf{Exercise \texttt{[unsupported Typst element: context]}.} \emph{Verify that \ensuremath{F}-algebras form a category.} \texttt{[unsupported Typst element: metadata]}  
\caption{}
\end{figure}
...
\begin{itemize}
\item In the case of identity, the component of \ensuremath{F\left( X \right)} we are focusing on is \ensuremath{F\left( X \right) = 1}. So both \ensuremath{F\left( X \right)} and \ensuremath{F\left( Y \right)} evaluate to \ensuremath{1}, so \ensuremath{F\left( X \right) \rightarrow F\left( Y \right)} can only be the unique identity morphism in \ensuremath{1}. Thus, the left side collapses and we have a triangle: \texttt{[unsupported Typst element: context]} Translating this to group theory, this means our choice of \ensuremath{{\alpha }_{Y}}, the function that picks out the group identity of \ensuremath{Y}, must agree with the result of mapping \ensuremath{f} to the group identity of \ensuremath{X}. In other words, group homomorphisms preserve identities.
\item In the case of inverse, \ensuremath{F\left( X \right) = X}, so we have the following diagram: \texttt{[unsupported Typst element: context]} This translates to \ensuremath{{\mathsf{inv}}_{Y} \circ f = f \circ {\mathsf{inv}}_{X}}, which written algebraically says that for any \ensuremath{x \in X}, it must hold that \ensuremath{{f\left( x \right)}^{-1} = f\left( {x}^{-1} \right)}. In other words, group homomorphisms preserve inverses.
\item In the case of the binary operation \ensuremath{\left( \cdot \right)}, we have \ensuremath{F\left( X \right) = X \times X}, so we have the following diagram (recall the definition of the action of \ensuremath{F} on morphisms): \texttt{[unsupported Typst element: context]} Translating this to algebra, we have \ensuremath{f\left( x {\cdot }_{X} y \right) = f\left( x \right) {\cdot }_{Y} f\left( y \right)}, which is the main group homomorphism property.
\end{itemize}
...
\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \textbf{Exercise \texttt{[unsupported Typst element: context]}.} \emph{Prove that \ensuremath{\mathsf{natrec}} is the unique morphism that makes the above square commute.} \texttt{[unsupported Typst element: metadata]}  
\caption{}
\end{figure}
...
\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \textbf{Exercise \texttt{[unsupported Typst element: context]}.} \emph{Define some other common algebraic structures using functors.} \texttt{[unsupported Typst element: metadata]}  
\caption{}
\end{figure}
...
\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \textbf{Exercise \texttt{[unsupported Typst element: context]}.} \emph{Prove that there is no non-empty initial algebra for the functor \ensuremath{F\left( X \right) = A \times X} in \ensuremath{\mathbf{Set}}, for any arbitrary object \ensuremath{A}. What does this mean in English?} \texttt{[unsupported Typst element: metadata]}  
\caption{}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{\ensuremath{F}-coalgebra}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{\ensuremath{F}-coalgebra morphism}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{Lambek"s theorem for coalgebra}
\end{figure}
 

\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \textbf{Exercise \texttt{[unsupported Typst element: context]}.} \emph{Prove \ref{lambekCoalgebra}.} \texttt{[unsupported Typst element: metadata]}  
\caption{}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{Bisimulation of streams}
\end{figure}
...
\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \textbf{Exercise \texttt{[unsupported Typst element: context]}.} \emph{Does the category of \ensuremath{F}-algebras have an initial object for any \ensuremath{F}? Does the category of \ensuremath{F}-coalgebras have a terminal object for any \ensuremath{F}?} \texttt{[unsupported Typst element: metadata]}  
\caption{}
\end{figure}
...
This visualization represents the formal automaton defined as \ensuremath{\left( \left\{ {q}_{0}, {q}_{1}, {q}_{2} \right\}, \left\{ \mathrm{[?raw]} , \mathrm{[?raw]} , \mathrm{[?raw]} \right\}, \delta , {q}_{0}, \left\{ {q}_{2} \right\} \right)} where \ensuremath{\delta} would map: \begin{itemize}
\item \ensuremath{\left( \mathrm{[?raw]} , {q}_{0} \right) \mapsto {q}_{2}}
\item \ensuremath{\left( \mathrm{[?raw]} , {q}_{0} \right) \mapsto {q}_{1}}
\item \ensuremath{\left( \mathrm{[?raw]} , {q}_{1} \right) \mapsto {q}_{1}}
\item \ensuremath{\left( \mathrm{[?raw]} , {q}_{1} \right) \mapsto {q}_{2}}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{Non-deterministic automaton}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{Category of deterministic automata, \ensuremath{{\mathsf{DA}}_{\mathcal{L}}}}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{Initial object of \ensuremath{{\mathsf{DA}}_{\mathcal{L}}}}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{Terminal object in \ensuremath{{\mathsf{DA}}_{\mathcal{L}}}}
\end{figure}
...
\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \textbf{Exercise \texttt{[unsupported Typst element: context]}.} \emph{Verify that \ensuremath{f} satisfies the requirements for being a \ensuremath{{\mathsf{DA}}_{\mathcal{L}}}-morphism.} \texttt{[unsupported Typst element: metadata]}  
\caption{}
\end{figure}
...
We will devise a functor called \ensuremath{\mathrm{[?overline]}}, which is self-dual, that makes this diagram commute:

\texttt{[unsupported Typst element: context]}

This essentially lifts the self-duality of \ensuremath{{\mathcal{P}}^{∗}} into \ensuremath{{\mathsf{DA}}_{\mathcal{L}}}. The paper \ref{bonchiAlgebracoalgebraDualityBrzozowskis2014} notes that this lifting is true generically

\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{Self-dual powerset functor lift}
\end{figure}
...
\begin{figure}[htbp]
\centering
\texttt{[unsupported Typst element: metadata]} \textbf{Exercise \texttt{[unsupported Typst element: context]}.} \emph{Prove \ref{oDALReflectiveSubcategory}. The proof should look mostly like a dualization of the proof of \ref{rDALCoreflectiveSubcategory}.} \texttt{[unsupported Typst element: metadata]}  
\caption{}
\end{figure}
...
\centering
\texttt{[unsupported Typst element: metadata]} \texttt{[unsupported Typst element: context]} \texttt{[unsupported Typst element: metadata]}  
\caption{Brzozowski minimization algorithm}
\end{figure}
...
\texttt{[unsupported Typst element: bibliography]} 
\end{document}
