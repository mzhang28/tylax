...
Of course, since \ensuremath{F} is a functor, it must have an action on morphisms as well. In the case of \ensuremath{F\left( X \right) = A \times X + B}, a functor application to a morphism \ensuremath{F\left( X \xrightarrow{f} Y \right)} would have type \ensuremath{A \times X + B \xrightarrow{F\left( f \right)} A \times Y + B}. It may be useful to think of \ensuremath{F\left( f \right)} as being a coproduct of morphisms \ensuremath{A \times X \xrightarrow{{f}_{1}} A \times Y} and \ensuremath{B \xrightarrow{{f}_{2}} B}. This is often true for the cases we will be studying, as many of the functors for the common algebras do exhibit this property. However, it may not be true in general, and the categorical framework for algebra allows for functors that mix between the different components. Still, we will use the notation \ensuremath{F\left( f \right) = {f}_{1} + {f}_{2}} in the remainder of the notes.
...
\begin{center}
\begin{tikzcd}
1 + X \arrow[r, "{\alpha}"] \arrow[d, "{F\left( f \right)}"] & X \arrow[d, "{f}"] \\
1 + Y \arrow[r, "{\beta}"'] & Y
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
F\left( X \right) \arrow[r, "{\alpha}"] \arrow[d, "{F\left( ! \right)}"] & X \arrow[d, "{!}"] \\
? \arrow[r, "{?}"'] & ? \\
{} & {}
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
{} \arrow[r, "{start}"] & {q}_{0} \arrow[r, "{b}"] \arrow[rr, bend right=40, "{a}"] & {q}_{1} \arrow[r, "{c}"] \arrow[loop above, bend left=130, "{b}"] & {q}_{2}
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
{q}_{0} \arrow[r, "{a}"] & {q}_{1} \arrow[r, "{a}"] & {q}_{2} \arrow[r, "{a}"] & {q}_{3} \arrow[lll, bend right=30, "{a}"]
\end{tikzcd}
\end{center}
...
\item \ensuremath{{Q}_{\mathsf{Init}}} is defined as \ensuremath{{\Sigma }^{\ast }}, the set of all strings over the alphabet \ensuremath{\Sigma}
...
First, define \ensuremath{f : {Q}_{\mathsf{Init}} \rightarrow {Q}^{'}}. For every state \ensuremath{q \in {Q}_{\mathsf{Init}}}, there is a corresponding string \ensuremath{s \in {\Sigma }^{\ast }}. We will define \ensuremath{f} to map \ensuremath{s} to a state in \ensuremath{{Q}^{'}} by running \ensuremath{{Q}^{'}} -- in other words, starting with \ensuremath{{i}^{'}} and transforming it using \ensuremath{{\delta }^{'}} on each symbol of \ensuremath{s}, and then returning the final state that it landed in.
...
\] Let \ensuremath{q \in {Q}_{\mathsf{Init}}} be an arbitrary state in \ensuremath{\mathsf{Init}}, and \ensuremath{s \in {\Sigma }^{\ast }} be the corresponding string. Then, we are trying to show that \ensuremath{f\left( {\delta }_{\mathsf{Init}} \left( q, t \right) \right) = {\delta }^{'}\left( f\left( q \right), t \right)}. Since \ensuremath{{\delta }_{\mathsf{Init}}} is defined by concatenation, we have \ensuremath{f\left( {\delta }_{\mathsf{Init}} \left( q, t \right) \right) = f\left( s + t \right)} on the left side. Since \ensuremath{f} simply runs \ensuremath{{\delta }^{'}} iteratively, this is actually true \emph{by the definition} of \ensuremath{f}.

It is trivial to show that \ensuremath{f} also preserves the initial and final states.

In order to show \ensuremath{f} is the unique such morphism, assume \ensuremath{g : {Q}_{\mathsf{Init}} \rightarrow {Q}^{'}}. We need to show for any state \ensuremath{q}, that \ensuremath{f\left( q \right) = g\left( q \right)}. Since the states map one-to-one with all strings \ensuremath{{\Sigma }^{\ast }}, perform induction on strings. For the empty string, the corresponding state is the initial state, which is preserved by the requirement of the automaton morphism. Thus, \ensuremath{f\left( {i}_{\mathsf{Init}} \right) = g\left( {i}_{\mathsf{Init}} \right) = {i}^{'}}.
...
Expanding this, we get: \ensuremath{Q \cong 1 + \Sigma \times \left( 1 + \Sigma \times \left( 1 + \Sigma \times \ldots \right) \right)}, which simplifies to \ensuremath{1 + \Sigma + {\Sigma }^{2} + {\Sigma }^{3} \ldots}. This is exactly \ensuremath{{\Sigma }^{\ast }} -- all strings of all lengths, sourced from the alphabet. \footnote{ In fact, this approach can be used to derive the initial automaton. I didn't realize this until after I wrote the previous proof\ldots{} However, I left the original proof in, because it actually defines the function needed for the morphism. }  ~\ensuremath{\square}

The rough design of this morphism is that for some arbitrary automaton \ensuremath{Q}, we're running every possible string in \ensuremath{{\Sigma }^{\ast }} to see which states are able to be reached via \ensuremath{Q}, and capturing that in a single function \ensuremath{f}.
...
\item \ensuremath{{Q}_{\mathsf{Term}}} is the set of all possible subsets of \ensuremath{{\Sigma }^{\ast }}. This represents all languages that could possibly be recognized for this alphabet.
...
Thus, \ensuremath{{2}^{{\Sigma }^{\ast }}} is the state space for the terminal automaton.  ~\ensuremath{\square}
...
\[
{\delta }_{{Q}^{'}} :\equiv \left( q, s \right) \mapsto \left\{ {q}^{'} \in Q \mid \delta \left( {q}^{'}, s \right) = q \right\}
\]
...
\[
{\delta }_{{Q}^{'}} :\equiv \left( q, s \right) \mapsto \left\{ {q}^{'} \in Q \mid \delta \left( {q}^{'}, s \right) \in q \right\}
\]\label{preImageDelta} 
...
\[
U\left( Q \right) \xrightarrow{{\mathcal{P}}^{\ast }} U\left( {Q}^{'} \right)
\]

Here we introduce a powerful property of the contravariant powerset functor:

\begin{lemma}
The contravariant powerset functor \ensuremath{{\mathcal{P}}^{\ast }} is left adjoint to its own opposite functor \ensuremath{{{\mathcal{P}}^{\ast }}^{\mathsf{op}}}.
\end{lemma}
\label{contrPowerSetSelfDual} 

\emph{Proof}.  The contravariant powerset functor is \ensuremath{{\mathcal{P}}^{\ast } : {\mathbf{Set}}^{\mathsf{op}} \rightarrow \mathbf{Set}}. Its opposite functor would be \ensuremath{{{\mathcal{P}}^{\ast }}^{\mathsf{op}} : \mathbf{Set} \rightarrow {\mathbf{Set}}^{\mathsf{op}}}. This amounts to showing that for any \ensuremath{X, Y \in \mathbf{Set}}:

\[
\begin{aligned}
{\mathbf{Hom}}_{\mathbf{Set}} \left( {\mathcal{P}}^{\ast } \left( X \right), Y \right) & \cong {\mathbf{Hom}}_{\mathbf{Set}} \left( X, {{\mathcal{P}}^{\ast }}^{\mathsf{op}}\left( Y \right) \right) \\ {\mathbf{Hom}}_{\mathbf{Set}} \left( Y, {\mathcal{P}}^{\ast }\left( X \right) \right) & \cong {\mathbf{Hom}}_{\mathbf{Set}} \left( X, {{\mathcal{P}}^{\ast }}^{\mathsf{op}}\left( Y \right) \right)
\end{aligned}
\]

and really, both \ensuremath{{\mathcal{P}}^{\ast }} and \ensuremath{\operatorname{{\mathcal{P}}^{\ast }}} have the same action on sets, which is to turn them into their power sets. So really, we are trying to show:
...
\[
\begin{aligned}
\forall g . g & = \mathsf{fwd}\left( \mathsf{bwd}\left( g \right) \right) \\ & = \lambda \left( x : X \right) . \left\{ y \in Y \mid x \in \mathsf{bwd}\left( g \right)\left( y \right) \right\} \\ & = \lambda \left( x : X \right) . \left\{ y \in Y \mid x \in \left( \lambda \left( y : Y \right) . \left\{ x \in X \mid y \in g\left( x \right) \right\} \right)\left( y \right) \right\} \\ & = \lambda \left( x : X \right) . \left\{ y \in Y \mid x \in \left\{ x \in X \mid y \in g\left( x \right) \right\} \right\} \\ & = \lambda \left( x : X \right) . \left\{ y \in Y \mid y \in g\left( x \right) \right\} \\ & = \lambda \left( x : X \right) . g\left( x \right) \\ & = g \\
\end{aligned}
\]  ~\ensuremath{\square}
...
\begin{center}
\begin{tikzcd}
{\mathsf{DA}}_{\mathcal{L}} \arrow[r, bend left=15, "{\overline{2}}"] \arrow[r, draw=none, "{\bot}"] \arrow[r, bend right=15, "{{\overline{2}}^{\mathsf{op}}}"'] \arrow[d, "{U}"] & {{\mathsf{DA}}_{\mathsf{rev}\left( \mathcal{L} \right)}}^{\mathsf{op}} \arrow[d, "{U}"] \\
\mathbf{Set} \arrow[r, bend left=15, "{{\mathcal{P}}^{\ast }}"] \arrow[r, draw=none, "{\bot}"] \arrow[r, leftarrow, bend right=15, "{{{\mathcal{P}}^{\ast }}^{\mathsf{op}}}"'] & {\mathbf{Set}}^{\mathsf{op}}
\end{tikzcd}
\end{center}


This essentially lifts the self-duality of \ensuremath{{\mathcal{P}}^{\ast }} into \ensuremath{{\mathsf{DA}}_{\mathcal{L}}}. The paper \cite{bonchiAlgebracoalgebraDualityBrzozowskis2014} notes that this lifting is true generically
...
\item action on morphisms maps a morphism \ensuremath{{Q}_{1} \xrightarrow{f} {Q}_{2}} to its inverse mapping \ensuremath{\mathcal{P}\left( {Q}_{2} \right) \xrightarrow{\overline{\mathcal{P}}\left( f \right)} \mathcal{P}\left( {Q}_{1} \right)}
\end{itemize}
...
\[
\begin{aligned}
{\left( \epsilon I \right)}_{{Q}^{'}} \circ {\left( I \eta \right)}_{{Q}^{'}} & = {\left( {1}_{I} \right)}_{{Q}^{'}} \\ I\left( {Q}^{'} \right) \xrightarrow{{\left( I \eta \right)}_{{Q}^{'}}} I\left( R\left( I\left( {Q}^{'} \right) \right) \right) \xrightarrow{{\left( \epsilon I \right)}_{{Q}^{'}}} I\left( {Q}^{'} \right) & = I\left( {Q}^{'} \right) \xrightarrow{{\left( {1}_{I} \right)}_{{Q}^{'}}} I\left( {Q}^{'} \right)
\end{aligned}
\]
...
\[
\begin{aligned}
{\left( \epsilon R \right)}_{Q} \circ {\left( R \eta \right)}_{Q} = {\left( {1}_{R} \right)}_{Q} \\ R\left( Q \right) \xrightarrow{{\left( R \eta \right)}_{Q}} R\left( I\left( R\left( Q \right) \right) \right) \xrightarrow{{\left( \epsilon R \right)}_{Q}} R\left( Q \right) = R\left( Q \right) \xrightarrow{{\left( {1}_{R} \right)}_{Q}} R\left( Q \right)
\end{aligned}
\]
...
\begin{center}
\begin{tikzcd}
{} & {\mathsf{DA}}_{\mathcal{L}} \arrow[dl, bend left=15, "{R}"] \arrow[dr, bend right=15, "{O}"] & {} \\
{\mathsf{rDA}}_{\mathcal{L}} \arrow[ur, hook, bend left=30] \arrow[ur, draw=none, bend left=10, "{\bot}"] \arrow[dr, draw=none, bend right=10, "{\top}"] \arrow[dr, bend right=30, "{O}"] & {} & {\mathsf{oDA}}_{\mathcal{L}} \arrow[ul, hook, bend right=30] \arrow[ul, draw=none, bend right=10, "{\top}"] \arrow[dl, draw=none, bend left=10, "{\bot}"] \arrow[dl, bend left=30, "{R}"] \\
{} & {\mathsf{mDA}}_{\mathcal{L}} \arrow[ul, hook, bend right=15] \arrow[ur, hook, bend left=15] & {}
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
{} &  \arrow[dl, bend left=15, "{R}"] \arrow[dr, bend right=15, "{O}"] & {} & {} & {{\mathsf{DA}}_{\mathsf{rev}\left( \mathcal{L} \right)}}^{\mathsf{op}} \arrow[dl, bend left=15, "{{R}^{\mathsf{op}}}"] \arrow[dr, bend right=15, "{{O}^{\mathsf{op}}}"] & {} \\
{\mathsf{rDA}}_{\mathcal{L}} \arrow[ur, hook, bend left=30] \arrow[ur, draw=none, bend left=10, "{\bot}"] \arrow[dr, draw=none, bend right=10, "{\top}"] \arrow[dr, bend right=30, "{O}"] \arrow[rrrrr, bend left=5, "{\overline{2}}"] & {} & {\mathsf{oDA}}_{\mathcal{L}} \arrow[ul, hook, bend right=30] \arrow[ul, draw=none, bend right=10, "{\top}"] \arrow[dl, draw=none, bend left=10, "{\bot}"] \arrow[dl, bend left=30, "{R}"] & {{\mathsf{rDA}}_{\mathsf{rev}\left( \mathcal{L} \right)}}^{\mathsf{op}} \arrow[ur, hook, bend left=30] \arrow[ur, draw=none, bend left=10, "{\bot}"] \arrow[dr, draw=none, bend right=10, "{\top}"] \arrow[dr, bend right=30, "{{O}^{\mathsf{op}}}"] & {} & {{\mathsf{oDA}}_{\mathsf{rev}\left( \mathcal{L} \right)}}^{\mathsf{op}} \arrow[ul, hook, bend right=30] \arrow[ul, draw=none, bend right=10, "{\top}"] \arrow[dl, draw=none, bend left=10, "{\bot}"] \arrow[dl, bend left=30, "{{R}^{\mathsf{op}}}"] \\
{} &  \arrow[ul, hook, bend right=15] \arrow[ur, hook, bend left=15] & {} & {} & {{\mathsf{mDA}}_{\mathsf{rev}\left( \mathcal{L} \right)}}^{\mathsf{op}} \arrow[ul, hook, bend right=15] \arrow[ur, hook, bend left=15] \arrow[lll, bend left=5, "{{\overline{2}}^{\mathsf{op}}}"] & {}
\end{tikzcd}
\end{center}
